...
{
    const auto player = nodePlayerControl.getOrCreatePlayer(NODE, std::nullopt);
    const auto player2 = nodePlayerControl.getOrCreatePlayer(NODE, std::nullopt);
    ASSERT_EQ(player, player2);
}

TEST_F(NodePlayerControlTest, testAutogeneratedPlayerByMultipleNodes)
{
    const auto player = nodePlayerControl.getOrCreatePlayer(NODE, std::nullopt);
    const auto player2 = nodePlayerControl.getOrCreatePlayer(OTHER_NODE, std::nullopt);
...
{
    nodePlayerControl.getOrCreatePlayer(NODE, UUID);
    nodePlayerControl.getOrCreatePlayer(NODE, OTHER_UUID);
    EXPECT_FALSE(nodePlayerControl.getOrCreatePlayer(NODE, std::nullopt));
}
